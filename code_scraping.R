...
part3 <- "&page="
#part4 <- "&public_timestamp%5Bfrom%5D=01%2F01%2F2018" #not for RESEARCH
part4 <- "" #for RESEARCH
...
counter_department <- 1


###START loop department search
while ( counter_department <=length(targets$department)){
  ...
  while ( search_page <= targets$pages_RA[ counter_department ]){
    pagelinks<-try(LinkExtractor(paste0( part1, type, part2, targets$department[ counter_department ], part3, counter_department , part4), urlExtfilter=urlExtfilter)[[2]])
    pagelinks<-pagelinks[!pagelinks%in%links_stop]
    links_html<-unique(c(links_html,pagelinks))
    
    links_html_new <- data.frame( links=links_html, dep = targets$Dep_acronyms[ counter_department ]) #collects links just collected and adds them to a dataframe with the department they’re from. For complete list of all HTML links produced in next step
    ...
    counter_department <- counter_department + 1 #move to next link when loop restarts
  }
  ...
  counter_html_doc <-1 #which html link to start looking for pdf links in
  ###START loop pdf link search
  while ( counter_html_doc <= length(links_html)){
    newlinks <- try(LinkExtractor(links_html[ counter_html_doc ], urlExtfilter=urlExtfilter_pdf)[[2]]) #find all links on current URL
    ...
    counter_html_doc <- counter_html_doc +1 #move to next HTML link when loop restarts
  }
  ...
  counter_pdf_doc <- 1 #which link in links_pdf to start from
  while ( counter_pdf_doc <=length(links_pdf)){
 #   while ( counter_pdf_doc <=180){
    infos<-try(pdf_info(links_pdf[ counter_pdf_doc ])) #extracts metadata from pdf
    ...
    newrow<-data.frame(
      URL=links_pdf[ counter_pdf_doc ],
      File=ifelse("Title" %in% names(infos$keys), infos$keys$Title, NA),
      Dep_ext=targets$Dep_acronyms[ counter_department ],
    ...
    text<-try(pdf_text(links_pdf[ counter_pdf_doc ]))
    ...
    counter_pdf_doc <- counter_pdf_doc +1
  }
  ...
  counter_department <- counter_department+1 #move to next department when loop restarts
}
